...
//come back on button click
    ...
    const sendAddedTask=()=>{
        if(date==""){
            dispatch(alertActions.error("Date is required"))
        }
        else if(task==""){
            dispatch(alertActions.error("Task is required"))
               ...
               setProjectTaskArray([...projectTaskArray,{date,taskid,projectid,hours,minutes,task}])
               setHours(0);
               setMinutes(0);
               setDate("");
               setTask("");
    ...
    return(
        <div className="project-details">
          ...
          <h1>{location?.state?.name}</h1>
              <TextBox title="Date" date={date} setDate={setDate} value={date} type={"date"}/> 
